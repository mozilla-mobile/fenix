...
class BackgroundServicesTest {
    ...
    fun `telemetry account observer does not track existing account`() {
        ...
        verify(inverse = true) { metrics.track(Event.SyncAuthSignIn) }
        verify(inverse = true) { metrics.track(Event.SyncAuthSignUp) }
        verify(inverse = true) { metrics.track(Event.SyncAuthPaired) }
        verify(inverse = true) { metrics.track(Event.SyncAuthFromShared) }
        verify(inverse = true) { metrics.track(Event.SyncAuthRecovered) }
        verify(inverse = true) { metrics.track(Event.SyncAuthOtherExternal) }
        verify(inverse = true) { metrics.track(Event.SyncAuthSignOut) }
        verify { settings.fxaSignedIn = true }
        confirmVerified(metrics, settings)
    }
