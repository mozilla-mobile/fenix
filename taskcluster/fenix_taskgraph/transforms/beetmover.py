...
beetmover_description_schema = Schema(
    {
        ...
        Required("worker"): {"upstream-artifacts": [{text_type: Any()}]},
...
def make_task_description(config, tasks):
    for task in tasks:
        ...
        description = (
            "Beetmover submission for locale '{locale}' for build type "
            "'{build_type}'".format(
...
def craft_release_properties(config, task):
    ...
    return {
        "app-name": ensure_text(params["project"].capitalize()),
